...
collateral_mult_test <- function(MIC_data, effect_type = "both",
  ...
  names_all <- c("A", "B", "n", "n_high", "tau", "mean_high", "mean_low")
  names_test_successful <- c("t", "p")
  ...
  for (dep in 1:m) {
    for (indep in 1:m) {
      ...
      # #Skip if the number of stains with both antibiotics is smaller than
      # MIC_select <- MIC_data[, c(dep, indep)]
      # if (sum(rowSums(is.na(MIC_select)) < 1) > 1) {
      #   next
      # }
      counter <- counter + 1
